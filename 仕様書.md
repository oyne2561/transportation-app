...
`project.clj` に spec の依存関係を追加します。
...
### ステップ1.2: spec の定義（オプション）

Clojure spec を使って、各エンティティのスキーマを定義します。
...
;; Order Status の定義
(s/def ::order-status #{:注文受付 :配達業者割り当て済み :配達中 :配達完了 :キャンセル})

;; Package の定義
(s/def ::width pos?)
(s/def ::height pos?)
(s/def ::depth pos?)
(s/def ::size (s/keys :req-un [::width ::height ::depth]))
(s/def ::weight pos?)
(s/def ::description string?)
(s/def ::package (s/keys :req-un [::size ::weight ::description]))

;; Order の定義
(s/def ::order-id string?)
(s/def ::customer-id string?)
(s/def ::order-datetime inst?)
(s/def ::from-address string?)
(s/def ::to-address string?)
(s/def ::desired-delivery-datetime inst?)

(s/def ::order (s/keys :req-un [::order-id
                                 ::customer-id
                                 ::order-datetime
                                 ::from-address
                                 ::to-address
                                 ::package
                                 ::status
                                 ::desired-delivery-datetime]))
```

**この段階では、spec の定義だけを行い、テストは書きません。**
**次のフェーズで、実際の関数を実装する際に spec を使います。**
...
(deftest validate-order-test
  (testing "正常な注文はバリデーション成功"
    (let [order {:from-address "東京都渋谷区..."
                 ...
                 :desired-delivery-datetime #inst "2024-01-02T14:00:00Z"}]
      (is (= {:valid? true} (order/validate-order order #inst "2024-01-01T10:00:00Z")))))
  ...
  (testing "配送元と配送先が同じ場合はエラー"
    (let [order {:from-address "東京都渋谷区..."
      ...
      (let [result (order/validate-order order #inst "2024-01-01T10:00:00Z")]
        (is (= {:valid? false :errors ["配送元と配送先が同じ住所です"]} result)))))
  ...
  (testing "希望配達日時が過去の場合はエラー"
    (let [order {:from-address "東京都渋谷区..."
      ...
      (let [result (order/validate-order order #inst "2024-01-01T10:00:00Z")]
        (is (= {:valid? false :errors ["希望配達日時は現在時刻より未来である必要があります"]} result)))))
  ...
  (testing "荷物の重量が上限を超える場合はエラー"
    (let [order {:from-address "東京都渋谷区..."
                 ...
                 :desired-delivery-datetime #inst "2024-01-02T14:00:00Z"}]
      (let [result (order/validate-order order #inst "2024-01-01T10:00:00Z")]
        (is (= {:valid? false :errors ["荷物の重量は30kg以下である必要があります"]} result)))))
...
(deftest cancel-order-test
  (testing "キャンセル可能な注文をキャンセルする"
    (let [order {:order-id "ORD-001"
                  :status :注文受付}]
      (let [result (order/cancel-order order)]
        (is (= :キャンセル (:status result)))
        (is (= "ORD-001" (:order-id result))))))
  
  (testing "キャンセル不可な注文をキャンセルしようとした場合はエラー"
    (let [order {:order-id "ORD-001"
                  :status :配達中}]
      (let [result (order/cancel-order order)]
        (is (= {:error "この注文はキャンセルできません"} result))))))
```
...
(deftest update-driver-status-test
  (testing "業者のステータスを更新する"
    (let [driver {:driver-id "DRV-001" :status :待機中}]
      (is (= :配達中 (:status (driver/update-driver-status driver :配達中))))))
  
  (testing "無効なステータスへの変更はエラー"
    (let [driver {:driver-id "DRV-001" :status :待機中}]
      (let [result (driver/update-driver-status driver :無効なステータス)]
        (is (= {:error "無効なステータスです"} result))))))
```
  ...
  (testing "注文を保存して取得できる"
    (let [repo (sd/->InMemoryOrderRepository {})
      ...
      (let [saved-repo (repo/save-order repo order)]
        (is (= order (repo/get-order saved-repo "ORD-001")))))))
```
  ...
  (testing "正常な注文を作成する"
    (let [repos (sd/create-repositories)
      ...
      (let [result (usecase/place-order-usecase repos params order-datetime)]
        (is (some? (:order-id result)))
        (is (= :注文受付 (:status result)))
        (is (= "CUST-001" (:customer-id result))))))
...
(deftest cancel-order-usecase-test
  ...
  (testing "存在しない注文IDの場合はエラー"
    (let [repos (sd/create-repositories)]
      (let [result (usecase/cancel-order-usecase repos "ORD-999")]
        (is (= {:error "注文が見つかりません"} result))))))
```
...
(deftest auto-assign-driver-usecase-test
  (testing "利用可能な配達業者を自動で割り当てる"
    (let [repos (sd/create-repositories)
      ...
      (let [result (usecase/auto-assign-driver-usecase repos "ORD-001")]
        (is (some? (:driver-id result)))
        (is (= :配達業者割り当て済み (:status result))))))
  ...
  (testing "利用可能な配達業者がいない場合はエラー"
    (let [repos (sd/create-repositories)
          ...
          repos (assoc repos :order-repo saved-order-repo)]
      (let [result (usecase/auto-assign-driver-usecase repos "ORD-001")]
        (is (= {:error "利用可能な配達業者が見つかりません"} result))))))
```
  ...
  (testing "注文から配達完了までの一連の流れ"
    (let [repos (sd/create-repositories)
      ...
      (let [order-params {:customer-id "CUST-001"
        ...
        (is (some? (:order-id order-result)))
        (is (= :注文受付 (:status order-result)))
        ...
        (let [repos (update repos :order-repo #(repo/save-order % order-result))
          ...
          (is (some? (:driver-id assign-result)))
          (is (= :割り当て済み (:status assign-result)))
          ...
          (let [delivery-repo (:delivery-repo repos)
            ...
            (is (= :配達中 (:status started-delivery)))
            ...
            (let [complete-datetime #inst "2024-01-02T11:00:00Z"
              ...
              (is (= :配達完了 (:status completed-delivery)))
              (is (= proof (:proof completed-delivery)))
