...
(defn place-order-usecase [repos params]
  (let [order-repo (:order-repo repos)]  ; ← Port層のProtocol
    (repo/save-order order-repo order)))  ; ← 実装は意識しない
```
...
(deftest place-order-usecase-test
  (testing "正常な注文を作成する"
    ...
    (mfn/providing [(repo/get-customer customer-repo "CUST-001") 
                     {:customer-id "CUST-001"
                      :name "山田太郎"
                      :address "東京都渋谷区..."}
                     (repo/save-order order-repo anything) 
                     (fn [repo order] order)]  ; 第2引数をそのまま返す
      (let [customer-repo (reify repo/CustomerRepository
                            (get-customer [this id] {:customer-id id}))
            order-repo (reify repo/OrderRepository
        ...
        (let [result (usecase/place-order-usecase repos params order-datetime)]
          ...
          (is (match? {:order-id string? 
                      ...
                      result))
          ;; verify で呼び出しを検証
          (mfn/verify (repo/get-customer customer-repo "CUST-001"))
          (mfn/verify (repo/save-order order-repo anything)))))))
```
...
#### ステップ5.2.2: リポジトリの実装（reify使用）

**ファイル**: `src/transportation_app/adapter/in_memory/repository.clj`

```clojure
(ns transportation-app.adapter.in-memory.repository
...
(defn create-repositories
  "すべてのリポジトリを作成して返す"
  []
  {:order-repo (create-order-repository)
   :customer-repo (create-customer-repository)
   :driver-repo (create-driver-repository)
   :delivery-repo (create-delivery-repository)})
```

**注意**: `reify`を使用する場合、状態管理は`atom`を使用します。`save-order`などのメソッドは状態を更新した後、`this`を返すことで、同じインスタンスを返します。

### ステップ5.3: Adapter層のテスト

**ファイル**: `test/transportation_app/adapter/in_memory/repository_test.clj`

```clojure
(ns transportation-app.adapter.in-memory.repository-test
  (:require [clojure.test :refer [deftest testing is]]
            [matcher-combinators.test :refer [match?]]
            [transportation-app.port.repository :as repo]
            [transportation-app.adapter.in-memory.repository :as adapter]))

(deftest order-repository-test
  (testing "注文を保存して取得できる"
    (let [order-repo (adapter/create-order-repository)
          order {:order-id "ORD-001" :status :注文受付}]
      (repo/save-order order-repo order)
      (is (match? order
                  (repo/get-order order-repo "ORD-001")))))
  
  (testing "ステータスで注文を検索できる"
    (let [order-repo (adapter/create-order-repository)
          order1 {:order-id "ORD-001" :status :注文受付}
          order2 {:order-id "ORD-002" :status :配達中}
          order3 {:order-id "ORD-003" :status :注文受付}]
      (repo/save-order order-repo order1)
      (repo/save-order order-repo order2)
      (repo/save-order order-repo order3)
      (let [orders (repo/find-orders-by-status order-repo :注文受付)]
        (is (= 2 (count orders)))
        (is (match? [{:order-id "ORD-001"} {:order-id "ORD-003"}]
                    (sort-by :order-id orders))))))
  
  (testing "顧客IDで注文を検索できる"
    (let [order-repo (adapter/create-order-repository)
          order1 {:order-id "ORD-001" :customer-id "CUST-001"}
          order2 {:order-id "ORD-002" :customer-id "CUST-002"}
          order3 {:order-id "ORD-003" :customer-id "CUST-001"}]
      (repo/save-order order-repo order1)
      (repo/save-order order-repo order2)
      (repo/save-order order-repo order3)
      (let [orders (repo/find-orders-by-customer-id order-repo "CUST-001")]
        (is (= 2 (count orders)))
        (is (match? [{:order-id "ORD-001"} {:order-id "ORD-003"}]
                    (sort-by :order-id orders)))))))

(deftest customer-repository-test
  (testing "顧客を取得できる"
    (let [customer-repo (adapter/create-customer-repository)]
      (is (match? {:customer-id "CUST-001"
                   :name "山田太郎"}
                  (repo/get-customer customer-repo "CUST-001")))))
  
  (testing "存在しない顧客IDの場合はnilを返す"
    (let [customer-repo (adapter/create-customer-repository)]
      (is (nil? (repo/get-customer customer-repo "CUST-999"))))))

(deftest driver-repository-test
  (testing "配達業者を取得できる"
    (let [driver-repo (adapter/create-driver-repository)]
      (is (match? {:driver-id "DRV-001"
                   :name "田中一郎"}
                  (repo/get-driver driver-repo "DRV-001")))))
  
  (testing "配達業者のステータスを更新できる"
    (let [driver-repo (adapter/create-driver-repository)
          driver (repo/get-driver driver-repo "DRV-001")
          updated-driver (assoc driver :status :配達中)]
      (repo/update-driver driver-repo updated-driver)
      (is (match? {:driver-id "DRV-001"
                   :status :配達中}
                  (repo/get-driver driver-repo "DRV-001")))))
  
  (testing "利用可能な配達業者を検索できる"
    (let [driver-repo (adapter/create-driver-repository)
          available-drivers (repo/find-available-drivers driver-repo "東京都新宿区")]
      (is (>= (count available-drivers) 1))
      (is (every? #(= :待機中 (:status %)) available-drivers))
      (is (every? #(clojure.string/includes? "東京都新宿区" (:assigned-area %)) available-drivers)))))

(deftest delivery-repository-test
  (testing "配達を保存して取得できる"
    (let [delivery-repo (adapter/create-delivery-repository)
          delivery {:delivery-id "DEL-001"
                    :order-id "ORD-001"
                    :status :未割り当て}]
      (repo/save-delivery delivery-repo delivery)
      (is (match? delivery
                  (repo/get-delivery delivery-repo "DEL-001")))))
  
  (testing "配達業者IDと期間で配達を検索できる"
    (let [delivery-repo (adapter/create-delivery-repository)
          start-date #inst "2024-01-01T00:00:00Z"
          end-date #inst "2024-01-01T23:59:59Z"
          delivery1 {:delivery-id "DEL-001"
                      :driver-id "DRV-001"
                      :start-datetime #inst "2024-01-01T10:00:00Z"}
          delivery2 {:delivery-id "DEL-002"
                      :driver-id "DRV-001"
                      :start-datetime #inst "2024-01-02T10:00:00Z"}
          delivery3 {:delivery-id "DEL-003"
                      :driver-id "DRV-002"
                      :start-datetime #inst "2024-01-01T10:00:00Z"}]
      (repo/save-delivery delivery-repo delivery1)
      (repo/save-delivery delivery-repo delivery2)
      (repo/save-delivery delivery-repo delivery3)
      (let [deliveries (repo/find-deliveries-by-driver-id delivery-repo "DRV-001" start-date end-date)]
        (is (= 1 (count deliveries)))
        (is (match? [{:delivery-id "DEL-001"}]
                    deliveries))))))

(deftest create-repositories-test
  (testing "すべてのリポジトリを作成できる"
    (let [repos (adapter/create-repositories)]
      (is (contains? repos :order-repo))
      (is (contains? repos :customer-repo))
      (is (contains? repos :driver-repo))
      (is (contains? repos :delivery-repo))
      (is (satisfies? repo/OrderRepository (:order-repo repos)))
      (is (satisfies? repo/CustomerRepository (:customer-repo repos)))
      (is (satisfies? repo/DriverRepository (:driver-repo repos)))
      (is (satisfies? repo/DeliveryRepository (:delivery-repo repos))))))
```
...
(deftest place-order-usecase-test
  ...
  (testing "存在しない顧客IDの場合はエラー"
    (let [customer-repo (reify repo/CustomerRepository
      ...
      (mfn/providing [(repo/get-customer customer-repo "CUST-999") nil]
...
(deftest cancel-order-usecase-test
  (testing "キャンセル可能な注文をキャンセルする"
    (let [order {:order-id "ORD-001"
      ...
      (mfn/providing [(repo/get-order order-repo "ORD-001") order
                       (repo/save-order order-repo anything) (fn [repo order] order)]
        (let [result (usecase/cancel-order-usecase repos "ORD-001")]
          (is (match? {:order-id "ORD-001"
                      :status :キャンセル}
                     result))
          (mfn/verify (repo/get-order order-repo "ORD-001"))
          (mfn/verify (repo/save-order order-repo anything))))))
  ...
  (testing "存在しない注文IDの場合はエラー"
    (let [order-repo (reify repo/OrderRepository
                       (get-order [this id] nil)
                       (save-order [this order] order))
          repos {:order-repo order-repo}]
      (mfn/providing [(repo/get-order order-repo "ORD-999") nil]
        (is (match? {:error "注文が見つかりません"}
                    (usecase/cancel-order-usecase repos "ORD-999")))
        (mfn/verify (repo/get-order order-repo "ORD-999"))))))
```
  ...
  (testing "利用可能な配達業者を自動で割り当てる"
    (let [order {:order-id "ORD-001"
          ...
          order-repo (reify repo/OrderRepository
                       (get-order [this id] order)
                       (save-order [this order] order))
          driver-repo (reify repo/DriverRepository
                        (find-available-drivers [this area] [driver])
                        (update-driver [this driver] driver))
          delivery-repo (reify repo/DeliveryRepository
                          (save-delivery [this delivery] delivery))
          repos {:order-repo order-repo
                 :driver-repo driver-repo
                 :delivery-repo delivery-repo}]
      (mfn/providing [(repo/get-order order-repo "ORD-001") order
                       (repo/find-available-drivers driver-repo "東京都新宿区新宿1-1-1") [driver]
                       (repo/save-delivery delivery-repo anything) (fn [repo delivery] delivery)
                       (repo/save-order order-repo anything) (fn [repo order] order)
                       (repo/update-driver driver-repo anything) (fn [repo driver] driver)]
        (let [result (usecase/auto-assign-driver-usecase repos "ORD-001")]
          (is (match? {:driver-id string?
                      :status :割り当て済み}
                     result))
          (mfn/verify (repo/get-order order-repo "ORD-001"))
          (mfn/verify (repo/find-available-drivers driver-repo "東京都新宿区新宿1-1-1"))
          (mfn/verify (repo/save-delivery delivery-repo anything)))))
  ...
  (testing "利用可能な配達業者がいない場合はエラー"
    (let [order {:order-id "ORD-001"
          ...
          repos {:order-repo order-repo
                 :driver-repo driver-repo}]
      (mfn/providing [(repo/get-order order-repo "ORD-001") order
                       (repo/find-available-drivers driver-repo "北海道札幌市...") []]
        (is (match? {:error "利用可能な配達業者が見つかりません"}
                    (usecase/auto-assign-driver-usecase repos "ORD-001")))
        (mfn/verify (repo/get-order order-repo "ORD-001"))
        (mfn/verify (repo/find-available-drivers driver-repo "北海道札幌市..."))))))
```
<｜tool▁calls▁begin｜><｜tool▁call▁begin｜>
read_file
...
(deftest end-to-end-scenario-test
  (testing "注文から配達完了までの一連の流れ"
    (let [repos (adapter/create-repositories)
      ...
      ;; 2. 配達業者を自動割り当て
      (let [order-repo (:order-repo repos)
            _ (repo/save-order order-repo order-result)
